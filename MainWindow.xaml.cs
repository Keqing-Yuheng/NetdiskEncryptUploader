...
namespace NetdiskEncryptUploader
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            {
                tblk_FilenameFormat.Text = "Filename Format: Relative";
            }
            else if (Varible.config.isForceAbsolutePath)
            {
                tblk_FilenameFormat.Text = "Filename Format: Absolute";
            }
            else
            {
                tblk_FilenameFormat.Visibility = Visibility.Hidden;
            }
            UpdateInfo();
            ...
            worker.RunWorkerCompleted += (s, e) =>
            {
                ...
                BackgroundWorker btnWorker = new BackgroundWorker();
                btnWorker.DoWork += (s, e) =>
                {
                    //等待压缩后台任务结束
                    if (Varible.config.isMultiThread)
                    {
                        while (countSuccessFile + countFailFile + countSkipFile != countTotalFile)  //读入文件数量=成功+失败+跳过文件数量时退出循环
                        {
                            Task.Delay(1);
                        }
                    }
                    //压缩结束 启用控件
                    isRun = false;
                    rect_IconRun.Dispatcher.BeginInvoke(new Action(() =>
                    {
                        rect_IconRun.Visibility = Visibility.Hidden;
                    }));
                    btn_Start.Dispatcher.BeginInvoke(new Action(() =>
                    {
                        btn_Start.IsEnabled = true;
                    }));
                    slider_CompressLv.Dispatcher.BeginInvoke(new Action(() =>
                    {
                        slider_CompressLv.IsEnabled = true;
                    }));
                    chkbox_Hash.Dispatcher.BeginInvoke(new Action(() =>
                    {
                        chkbox_Hash.IsEnabled = true;
                    }));
                    chkbox_Encrypt.Dispatcher.BeginInvoke(new Action(() =>
                    {
                        chkbox_Encrypt.IsEnabled = true;
                    }));
                    switch (args.Cancel)
                    {
                        case true:
                            rect_IconCancelled.Dispatcher.BeginInvoke(new Action(() =>
                            {
                            ...
                            break;
                        case false:
                            rect_IconComplete.Dispatcher.BeginInvoke(new Action(() =>
                            {
                ...
                btnWorker.RunWorkerAsync(btnWorker);
            };
        ...
        private void windowClosing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            {
                e.Cancel = true;
                MessageBox.Show("To close the window, wait until the task finishes", "NetdiskEncryptUploader", MessageBoxButton.OK, MessageBoxImage.Information);
            }
        }
        ...
        private void slider_Del_ValueChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
        {
            ...
            if (e.NewValue == 100) 
            {
                ...
                {
                    delErr = true;
                    Func.WriteLog(LogLevel.ERROR, "Unable to clear output: " + ex.Message);
                }
                
                switch(delErr)
                {
                    case true:  //删除过程出现异常
                        rect_IconWarnForClear.Visibility = Visibility.Visible;
                        break;
                    case false: //删除正常
        ...
        {
            rect_IconCompleteForClear.Visibility = Visibility.Hidden;
            rect_IconWarnForClear.Visibility = Visibility.Hidden;
        }
        ...
        {
            if(isRun)   //在运行状态下点击 则是取消
            {
                btn_Start.IsEnabled = false;    //在按下STOP之后 实际取消之前 禁用按钮 避免重复点击
                worker.CancelAsync();
                return;
            }
            ...
            rect_IconWarn.Visibility = Visibility.Hidden;
            //禁用设置修改按钮
            slider_CompressLv.IsEnabled = false;
            chkbox_Hash.IsEnabled = false;
            chkbox_Encrypt.IsEnabled = false;
        ...
        public void ProcessItem()
        {
            ...
            while (!listReader.EndOfStream)
            {
                ...
                {
                    //显示警告
                    rect_IconWarn.Dispatcher.BeginInvoke(new Action(() =>
                    {
                        rect_IconWarn.Visibility = Visibility.Visible;
                    }));
                    //增加计数
                    countTotalItem++;
                    countFailItem++;
                ...
                switch (type)
                {
                        ...
                        if (Directory.Exists(path))
                        {
                            ...
                            foreach(var file in files)
                            {
                                ...
                                if (Varible.config.isForceRelativePath)
                                {
                                    ...
                                    //获取目录长度 以分离出后面的部分
                                    dirLength = Path.GetFullPath(path).Length + 1;  //目录长度可直接作为SubString的StartIndex 从\的后一个字符开始
                                }
                    ...
                    case 'F':
                        ...
                        {
                            //获取文件路径中前面的目录部分的长度
                            string[] path_sub = path.Split('\\');
                            dirLength = 0;
                            for (int i = 0; i < path_sub.Length - 1; i++)   //最后分割出的一项是文件名 需要排除掉 故-1
                            {
                                dirLength += path_sub[i].Length;
                                dirLength++;    //反斜杠
                            }
                        }
                        //检查是否强制使用绝对路径(优先级低)
        ...
        public int ProcessSingleFile(string path)
        {
            ...
            //检测是否强制使用相对路径(优先级高于强制绝对路径) 若是则分割path
            ...
            if (Varible.config.isEncryptEnabled && isModified)
            {
                try
                {
                    ...
                    compressor.CompressionFinished += (sender, eventArgs) =>
                    {
                        try
                        {
                            //如果压缩成功且启用Hash 则在Hash文件末尾添加Done标记
                            if (Varible.config.isHashEnabled)
                            {
                        ...
                        if (Varible.config.isMultiThread)
                        {
                            ...
                            countOngoingFile = countOngoingFile < 0 ? 0 : countOngoingFile; //防止countOngoingFile有时候变成负数
                        }
                        countSuccessFile++;
                        UpdateStatistic(path);

                        //读入结束(或中止)且无正在处理的文件 则可判定不在运行状态
                        if (countSuccessFile + countFailItem + countSkipFile == countTotalFile)
                        {
                            isRun = false;
                        }
                    };
                    //开始压缩
                    switch(Varible.config.isMultiThread)
                    {
                        case true:
                            compressor.CompressFilesEncryptedAsync(compress_path, Varible.config.passwd, path);
                            break;
                        case false:
        ...
        public void UpdateInfo()
        {
            tblk_Info.Text = "Input: " + Varible.config.inputList
                           ...
                           + "\nMulti-thread: " + (Varible.config.isMultiThread ? "ON" : "OFF")
                           + "\nCompression Level: " + Varible.config.compressionLevel.ToString() + " / " + Func.GetCompressLvName(Varible.config.compressionLevel);
        }
        public void UpdateStatistic(string path)
        {
            //显示统计信息(避免"调用线程无法访问此对象，因为另一个线程拥有该对象"的错误)
            tblk_Progress.Dispatcher.BeginInvoke(new Action(() =>
            {
                tblk_Progress.Text = "Current: " + path
